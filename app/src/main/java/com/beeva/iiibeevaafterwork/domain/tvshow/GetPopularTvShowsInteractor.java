...
public class GetPopularTvShowsInteractor extends BaseInteractor {

    private final TvShowRepository tvShowRepository;

    public GetPopularTvShowsInteractor(@NonNull Context context) {
        this(TMDbApplication.from(context).getJobExecutor(),
                TMDbApplication.from(context).getUiExecutor(),
                new TvShowRepository(context));
    }

    GetPopularTvShowsInteractor(JobExecutor jobExecutor,
    ...
    private void runBackground(Callback<List<TvShow>> callback,
        ...
        try {
            List<TvShow> tvShows = tvShowRepository.getPopular();
